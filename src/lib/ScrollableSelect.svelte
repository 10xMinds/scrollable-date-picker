<script lang="ts">
	...
	function highlightTarget(index: number) {
		for (const element of scrollContainer.children) {
			...
			element.classList.remove('scale-[1.8]');
		...
		scrollContainer.children[index + itemPadding].classList.add('scale-[1.8]');
...
<div class="flex w-full overflow-y-auto text-center">
	<div
	...
	>
		<ul
			...
			class="no-scrollbar snap-y snap-mandatory overflow-scroll"
			on:scroll={handleScroll}
		>
			<!--  eslint-disable @typescript-eslint/no-unused-vars -->
			{#each { length: itemPadding } as _}
				<li class="h-16 snap-center snap-normal"></li>
			{/each}
			{#each items as item}
				<li
					class="h-16 text-xs snap-center snap-normal content-center text-center font-light transition-all"
				>
					{item.name}
				</li>
			{/each}
